...
const props = withDefaults(defineProps<BaseChartProps<T> & {
  ...
  /**
   * Display names of the categories
   */
  displayNames?: string[]
}>(), {
...
const legendItems = ref<(BulletLegendItemInterface & { displayName: string })[]>(props.categories.map((category, i) => ({
  name: category,
  displayName: props.displayNames?.[i] ?? category,
...
<template>
  <div :class="cn('w-full h-[400px] flex flex-col items-end', $attrs.class ?? '')">
    ...
    <VisXYContainer :style="{ height: isMounted ? '100%' : 'auto' }" :margin="{ left: 0, right: 0 }" :data="data">

            ...
            <div class="flex justify-center">
                ...
                <div class="rounded-t-3xl w-full max-w-56 portrait:max-w-48 relative bg-background/80 text-foreground">
                    ...
                    <header class="p-4 font-sf-pro-display font-semibold text-xs flex justify-between items-center">
                        <!-- Время -->
                        <p>9:41</p>
                        <!-- Иконки статуса: сеть, WiFi, батарея -->
...
/* Анимация появления диалогового окна с лисой */
@keyframes slide-up-fade {
  0% {
    opacity: 0;
    transform: translateY(40px) scale(0.95); /* Сдвиг снизу + небольшое уменьшение */
  }
  100% {
    opacity: 1;
    transform: translateY(0) scale(1); /* Финальная позиция */
  }
}

/* Анимация появления кнопок ответов */
@keyframes button-appear {
  0% {
    opacity: 0;
    transform: translateY(20px) scale(0.9); /* Легкий сдвиг снизу + уменьшение */
  }
  100% {
    opacity: 1;
    transform: translateY(0) scale(1); /* Финальная позиция */
